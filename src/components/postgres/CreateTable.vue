...
import {
    NIcon, NButton, NInput, NInputNumber, NCheckbox, NSelect, NSpin, NModal, NTable, NTabs, NTabPane, NPopover
} from 'naive-ui'
import { Add, ArrowDown, ArrowUp, Trash } from '@vicons/ionicons5'
...
const table = ref<any>({
    ...
    columns: [{
        ...
        collation1: 'public',
        collation2: '123',
        virtualType: 'GENERATED ALWAYS AS STORED',
        increment: 0,
        minvalue: 0,
        maxvalue: 0,
        start: 0,
...
const sqlCreateTable = computed(() => {
    ...
    table.value.columns.forEach((item: any) => {
        if (item.name && item.type) {
            ...
            if (item.virtualType) {
                if (item.virtualType == 'GENERATE ALWAYS AS IDENTITY') {
...
<template>
    <div style="padding: 14px">
        <n-modal v-model:show="showIndexFields" preset="card" style="width: 1200px;" :title="t('info')" size="small">
        ...
        <n-spin size="large" :show="loadingCreateTable">
            <n-input size="small" v-model:value="tablename" placeholder="Table Name" clearable />
            <n-tabs type="card" size="small">
                <n-tab-pane display-directive="show" key="column" name="column">
                    <n-table size="small" :single-line="false">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 140px">Decimal Digits</th>
                                <th style="width: 74px">Is Null</th>
                ...
                <n-tab-pane display-directive="show" key="index" name="index">
                    <n-table size="small" :single-line="false">
                        <thead>
                            <tr>
                                <th style="width: 200px">Name</th>
                            ...
                            <tr v-for="(i, j) in index">
                                ...
                                <td>
                                    <n-popover trigger="hover">
                                        ...
                                        <pre>
{{ formatIndexFields(i.fields).join('\n') }}
                                        </pre>
                                    </n-popover>
                                </td>
                                <td>
